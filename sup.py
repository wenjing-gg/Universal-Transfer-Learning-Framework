...
class SwinUNETRClassifier(nn.Module):
    """
    Swin UNETR 修改版，用于 3D CT 图像的二分类任务。
    """
    ...
    def __init__(
        ...
        spatial_dims: int = 3,
        norm_name: Union[Tuple, str] = "instance",  # 添加 norm_name
    ) -> None:
        """
        Args:
            img_size: 输入图像的尺寸。
            in_channels: 输入通道数。
            num_classes: 分类的类别数（对于二分类任务，num_classes=2）。
            depths: 每个阶段的层数。
            num_heads: 注意力头的数量。
            feature_size: 网络特征的维度。
            drop_rate: dropout 率。
            attn_drop_rate: 注意力 dropout 率。
            dropout_path_rate: 路径 dropout 率。
            use_checkpoint: 是否使用梯度检查点。
            spatial_dims: 空间维度（对于 3D 图像，spatial_dims=3）。
            norm_name: 归一化层类型。
        """
        super().__init__()
        ...
        # 全连接层，用于分类
        self.classifier = nn.Linear(16 * feature_size, num_classes)
    ...
    def forward(self, x_in):
        ...
        # 使用 enc4 进行分类
        pooled = self.global_pool(enc4)  # [B, C, 1, 1, 1]
...
if __name__ == '__main__':
    model = SwinUNETRClassifier(
        img_size=(64, 64, 64),  # 根据您的输入尺寸调整
        in_channels=1,          # 输入通道数
        num_classes=2,          # 分类数
        feature_size=48,        # 根据需要调整
        spatial_dims=3          # 对于 3D 图像
    )

    input_tensor = torch.randn(2, 1, 64, 64, 64)  # 示例输入张量
    output = model(input_tensor)
    print(output.shape)  # 应输出 torch.Size([2, 2])
